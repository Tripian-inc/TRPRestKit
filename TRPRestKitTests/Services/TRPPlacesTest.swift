...
class TRPPlacesTest: XCTestCase {
    ...
    func testFethPlaceWithCityId() {
        ...
        TRPRestKit().poi(withCityId: 112) { (result, error, pagination) in
            if let error = error {
...
//        wait(for: [expectation], timeout: 10.0)
    ...
    func testFethPlacesWithLocation() {
        ...
        TRPRestKit().poi(withLocation: TRPLocation(lat: 41, lon: 29)) { (result, error, pagination) in
            if let error = error {
                XCTFail("\(nameSpace) Parser Fail: \(error.localizedDescription)")
                return
            }
    ...
    func testFethPlacesWithLocationWithTypeId() {
        let nameSpace = "TRPRestKit().city(withPlacesId:)"
        let expectation = XCTestExpectation(description: "\(nameSpace) expectation")
        TRPRestKit().poi(withLocation: TRPLocation(lat: 41, lon: 29), typeId: 29) { (result, error, pagination) in
            if let error = error {
                XCTFail("\(nameSpace) Parser Fail: \(error.localizedDescription)")
                return
            }
